...
class Track:
    ...
    def __init__(
        self, detection, track_id, class_id, conf, n_init, max_age, ema_alpha, feature=None
    ):
    ...
    def ECC(
        ...
        try:
            (cc, warp_matrix
            ) = cv2.findTransformECC(src_r, dst_r, warp_matrix, warp_mode, criteria, None, 1)
        ...
        ) if warp_mode == cv2.MOTION_HOMOGRAPHY else cv2.warpAffine(
            src, warp_matrix, (sz[1], sz[0]), flags=cv2.INTER_LINEAR
        )

...
class Tracker:
    ...
    def _full_cost_metric(self, tracks, dets, track_indices, detection_indices):
        ...
        for row, track_idx in enumerate(track_indices):
            pos_cost[row, :] = np.sqrt(
                self.kf.
                gating_distance(tracks[track_idx].mean, tracks[track_idx].covariance, msrs, False)
    ...
    def _match(self, detections):
        ...
        def gated_metric(tracks, dets, track_indices, detection_indices):
            ...
            cost_matrix = linear_assignment.gate_cost_matrix(
                cost_matrix, tracks, dets, track_indices, detection_indices, self.mc_lambda
            )
    ...
    def _initiate_track(self, detection, class_id, conf):
        self.tracks.append(
            Track(
                detection.to_xyah(), self._next_id, class_id, conf, self.n_init, self.max_age,
                self.ema_alpha, detection.feature
            )
        )
        self._next_id += 1
